...
namespace GLTFast.Export
{
    ...
    public class GltfWriter : IGltfWritable
    {
        ...
        public void AddExtrasToNode(int nodeId, List<Dictionary<string, object>> extrasData)
        {
                if (nodeId < 0 || nodeId >= m_Nodes.Count)
                {
                    Debug.LogError($"Invalid node ID {nodeId} when adding extras.");
                    return;
                }

                var node = m_Nodes[nodeId];
                var extrasToken = JToken.FromObject(extrasData);
                node.extras = extrasToken;
        }
    }
}
